...
public class OCRService {
    ...
    public Map OCR(String json, HttpServletRequest request) {        
        ...
        response = customFunctions.validateArtifactKeyID(map.get("artifactKey").toString());
        if (response != null) {
            audit.setResponse(gson.toJson(response));
            auditService.save(audit);
            return response;
        ...
        if (!(caseTable.getAccounts().getAccountID() == tokenTable.getAccounts().getAccountID())) {
            ...
            OcrMap[] ocrMaps = gson.fromJson(artifact.getFeature_map(), OcrMap[].class);
            UznFormat uzn = new UznFormat();
            String path = artifact.getRepositoryRef();
            String conversionPath = System.getProperty( "catalina.base")+"/kycengine_artifacts/ocr_conversions/";
            ...
            try {
                ...
                if(uzn.createUZNFile(ocrMaps,conversionPath,fileName)){
                    ...
                    if(ocr_results == null){
                        response = customFunctions.updateTokenResponse("401", Constants.Errors.ONE.toString(), false);
                        audit.setResponse(gson.toJson(response));
                        auditService.save(audit);
                        return response;
                    }
